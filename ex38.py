...
def add_to_file():
    ...
    try:
        new_info = []
        directives = ['Введите фамилию: ', 'Введите имя: ',
                      'Введите отчество: ', 'Введите номер телефона: ']
...
def change_file():
    try:
        ...
        phonebook = phonebook.replace(
            changing_line, ' '.join(changing_line_list) + '\n')
...
def delete_line():
    try:
        ...
        confirm = input(
            f'Для подтверждения удаления данных нажмите 1, для отмены #: ')
